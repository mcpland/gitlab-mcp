...
const envSchema = z.object({
  ...
  MCP_METRICS_ENABLED: z.enum(["true", "false"]).default("false"),
  MCP_METRICS_AUTH_TOKEN: optionalHttpAuthToken,
  ...
  GITLAB_HTTP_RETRY_BASE_MS: z.coerce.number().int().min(0).max(10_000).default(250),
  GITLAB_HTTP_RETRY_MAX_DELAY_MS: z.coerce.number().int().min(0).max(120_000).default(10_000),
  ...
  MAX_REQUESTS_PER_MINUTE_PER_IP: z.coerce.number().int().min(1).max(10_000).default(300),
...
export const env = {
  ...
  MCP_TRUST_PROXY: parseBoolean(data.MCP_TRUST_PROXY, false),
  MCP_METRICS_ENABLED: parseBoolean(data.MCP_METRICS_ENABLED, false),
  ...
  GITLAB_ENABLE_COMPATIBILITY_ALIASES: parseBoolean(
    data.GITLAB_ENABLE_COMPATIBILITY_ALIASES,
    false
  ),
